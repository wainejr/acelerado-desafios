/*
 * Crystalline Lineage
 * @prompt prompts/prompt-m11-tv-denoise.md (revisão 1)
 * @module M11
 * @language c
 * @updated 2026-05-27
 */
...
void tv_denoise(
    ...
    // 4. Inicialização (x = input)
    memcpy(X_VAL, input, sizeof(float) * NN);
    ...
    for (int k = 0; k < n_iter; k++) {
        ...
        for (int i = 0; i < NN; i++) {
            ...
            float complex rhs_val = Z_FFT[i] + rho * (term_x + term_y);
